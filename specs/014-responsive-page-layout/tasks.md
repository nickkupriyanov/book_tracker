...
- **Acceptance:** Tests verify the ready non-empty home page still renders Reading Calendar only when expected, keeps calendar before shelf in DOM order, and exposes a stable responsive layout container.
...
- **Acceptance:** Tests verify found, loading, and not-found detail states render inside the shared page spacing container without changing their existing user-visible content.
...
- **Acceptance:** Home and book detail pages share the same outer horizontal padding, vertical padding, and constrained max-width rhythm.
...
- **Acceptance:** Ready non-empty home state is mobile vertical with calendar before shelf and desktop shelf-left/calendar-right with a sticky calendar rail.
...
- **Acceptance:** Existing add/edit/delete/filter/sort/rating/review/quote/reading-day behavior remains unchanged by the layout work.
...
- **Acceptance:** Mobile and desktop manual checks pass, `npm run lint` passes, and `npm run test` passes.
